...
export default function Forum(){
  ...
  return (
    <div className="space-y-4">
      <Card title="Forum" actions={
        <button onClick={async()=>{ setLoading(true); setError(null);
          try { await api.post('/forum/posts', form); setForm({} as any); await load() }
      ...
      <Card title="New">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <input className="border rounded-md px-3 py-2" placeholder="content" type="text" value={form["content"]||""} onChange={e=>setForm((p:any)=>({...p, "content": e.target.value}))} />
        </div>
      </Card>
