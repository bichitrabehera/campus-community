...
export default function Clubs(){
  ...
  return (
    <div className="space-y-4">
      <Card title="Clubs" actions={
        <button onClick={async()=>{ setLoading(true); setError(null);
          try { await api.post('/clubs', form); setForm({} as any); await load() }
      ...
      <Card title="New">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input className="border rounded-md px-3 py-2" placeholder="name" type="text" value={form["name"]||""} onChange={e=>setForm((p:any)=>({...p, "name": e.target.value}))} />
          ...
          <input className="border rounded-md px-3 py-2" placeholder="category" type="text" value={form["category"]||""} onChange={e=>setForm((p:any)=>({...p, "category": e.target.value}))} />
        </div>
      </Card>
