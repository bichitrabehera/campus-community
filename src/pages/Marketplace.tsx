...
export default function Marketplace(){
  ...
  return (
    <div className="space-y-4">
      <Card title="Marketplace" actions={
        <button onClick={async()=>{ setLoading(true); setError(null);
          try { await api.post('/marketplace', form); setForm({} as any); await load() }
      ...
      <Card title="New">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <input className="border rounded-md px-3 py-2" placeholder="price" type="text" value={form["price"]||""} onChange={e=>setForm((p:any)=>({...p, "price": e.target.value}))} />
        </div>
      </Card>
